...
def pdp_plot(pdp_isolate_out, feature_name, center=True, plot_pts_dist=False, plot_lines=False, frac_to_plot=1,
    ...
    if len(pdp_plot_data) == 1:
        ...
        if plot_pts_dist:
            ...
            x,y,std = _pdp_plot(pdp_isolate_out=pdp_plot_data[0], feature_name=feature_name_adj, pdp_ax=_pdp_ax,
                      count_ax=_count_ax, **pdp_plot_params)
        else:
            pdp_ax = plt.subplot(outer_grid[1])
            fig.add_subplot(pdp_ax)
            x,y,std = _pdp_plot(pdp_isolate_out=pdp_plot_data[0], feature_name=feature_name_adj, pdp_ax=pdp_ax,
        ...
        if plot_pts_dist:
            ...
            for inner_idx in range(len(pdp_plot_data)):
                ...
                x,y,std=_pdp_plot(pdp_isolate_out=pdp_plot_data[inner_idx], feature_name=feature_name_adj, pdp_ax=_pdp_ax,
                          count_ax=_count_ax, **pdp_plot_params)
        else:
            pdp_ax = []
            for inner_idx in range(len(pdp_plot_data)):
                ...
                feature_name_adj = '%s (class %d)' % (feature_name, pdp_plot_data[inner_idx].which_class)
                x,y,std=_pdp_plot(pdp_isolate_out=pdp_plot_data[inner_idx], feature_name=feature_name_adj, pdp_ax=ax,
                          count_ax=None, **pdp_plot_params)

    axes = {'title_ax': title_ax, 'pdp_ax': pdp_ax}

    return fig, axes,x,y,std#
